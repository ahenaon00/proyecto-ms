...
const api = axios.create({
  baseURL: "http://18.218.198.65:8080",
  headers: {
...
export const itemsAPI = {
  ...
  getItems: (filters?: {
    ...
    return api.get<ItemResponse[]>(
      `/marketplace-ms/items${queryString ? `?${queryString}` : ""}`
    );
  },
  ...
  createItem: (item: CreateItemRequest) =>
    api.post<ItemResponse>("/marketplace-ms/items", item),

  // Crear nuevo item con datos detallados (requiere JWT PROVEEDOR)
  createDetailedItem: (itemData: any) =>
    api.post<ItemResponse>("/marketplace-ms/items/detailed", itemData),

  // Actualizar item (requiere JWT PROVEEDOR)
  updateItem: (id: number, item: CreateItemRequest) =>
    api.put<ItemResponse>(`/marketplace-ms/items/${id}`, item),
  ...
  addToCart: (id: number, data: any) =>
    api.post(`/marketplace-ms/items/${id}/add-to-cart`, data),

  // Buscar items
  searchItems: (query: string) =>
    api.get<ItemResponse[]>(
      `/marketplace-ms/items/search?query=${encodeURIComponent(query)}`
    ),

  // Crear item completo con id
  createFullItem: (itemData: any) =>
    api.post<ItemResponse>("/marketplace-ms/items", itemData),
};

// API functions para preguntas frecuentes
export const preguntasAPI = {
  // Obtener preguntas de un item
  getPreguntas: (itemId: number) =>
    api.get<PreguntaFrecuente[]>(`/marketplace-ms/items/${itemId}/preguntas`),

  // Crear nueva pregunta (requiere JWT PROVEEDOR)
  createPregunta: (itemId: number, pregunta: { pregunta: string }) =>
    api.post<PreguntaFrecuente>(
      `/marketplace-ms/items/${itemId}/preguntas`,
      pregunta
    ),
...
export const reviewsAPI = {
  ...
  getReviews: (itemId: number) =>
    api.get<Review[]>(`/marketplace-ms/items/${itemId}/reviews`),

  // Crear nueva reseña
  createReview: (
    itemId: number,
    review: { puntuacion: number; titulo: string; comentario: string }
  ...
  createReply: (itemId: number, reviewId: number, reply: { cuerpo: string }) =>
    api.post<Reply>(
      `/marketplace-ms/items/${itemId}/reviews/${reviewId}/replies`,
      reply
    ),
};
...
export const servicesAPI = {
  ...
  createAlojamiento: (data: any) =>
    api.post("/marketplace-ms/alojamiento", data),

  // Crear servicio de alimentación
  createAlimentacion: (data: any) =>
    api.post("/marketplace-ms/alimentacion", data),
  ...
  createPaseosEcologico: (data: any) =>
    api.post("/marketplace-ms/paseos-ecologico", data),
};
